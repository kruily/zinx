...
type Config struct {
	/*
		Server
	*/
	Host    string `json:",default=0.0.0.0"`       // The IP address of the current server. (当前服务器主机IP)
	TCPPort int    `json:",default=8999"`          // The port number on which the server listens for TCP connections.(当前服务器主机监听端口号)
	WsPort  int    `json:",default=9000"`          // The port number on which the server listens for WebSocket connections.(当前服务器主机websocket监听端口)
	WsPath  string `json:",default=/"`             // The path of the WebSocket connection.(websocket连接路径)
	Name    string `json:",default=ZinxServerApp"` // The name of the current server.(当前服务器名称)
	KcpPort int    `json:",default=9001"`          // he port number on which the server listens for KCP connections.(当前服务器主机监听端口号)

	/*
		ServerConfig
	*/
	KcpACKNoDelay      bool `json:",default=false"` // changes ack flush option, set true to flush ack immediately,
	KcpStreamMode      bool `json:",default=true"`  // toggles the stream mode on/off
	KcpNoDelay         int  `json:",default=1"`     // Whether nodelay mode is enabled, 0 is not enabled; 1 enabled.
	KcpInterval        int  `json:",default=10"`    // Protocol internal work interval, in milliseconds, such as 10 ms or 20 ms.
	KcpResend          int  `json:",default=1"`     // Fast retransmission mode, 0 represents off by default, 2 can be set (2 ACK spans will result in direct retransmission)
	KcpNc              int  `json:",default=0"`     // Whether to turn off flow control, 0 represents “Do not turn off” by default, 1 represents “Turn off”.
	KcpSendWindow      int  `json:",default=32"`    // SND_BUF, this unit is the packet, default 32.(发送窗口大小，单位是包，默认32)
	KcpRecvWindow      int  `json:",default=32"`    // RCV_BUF, this unit is the packet, default 32.(接收窗口大小，单位是包，默认32)
	KcpFecDataShards   int  `json:",default=0"`     // The number of data shards in the FEC.(FEC数据分片), default 0.(默认0不开启FEC)
	KcpFecParityShards int  `json:",default=0"`     // The number of parity shards in the FEC.(FEC校验分片) default 0.(默认0不开启FEC)

	/*
		Zinx
	*/
	Version          string `json:",default=v1.0"`  // The version of the Zinx framework.(当前Zinx版本号)
	MaxPacketSize    uint32 `json:",default=4096"`  // The maximum size of the packets that can be sent or received.(读写数据包的最大值)
	MaxConn          int    `json:",default=12000"` // The maximum number of connections that the server can handle.(当前服务器主机允许的最大连接个数)
	WorkerPoolSize   uint32 `json:",default=10"`    // The number of worker pools in the business logic.(业务工作Worker池的数量)
	MaxWorkerTaskLen uint32 `json:",default=1024"`  // The maximum number of tasks that a worker pool can handle.(业务工作Worker对应负责的任务队列最大任务存储数量)
	WorkerMode       string `json:",optional"`      // The way to assign workers to connections.(为连接分配worker的方式)
	MaxMsgChanLen    uint32 `json:",default=1024"`  // The maximum length of the send buffer message queue.(SendBuffMsg发送消息的缓冲最大长度)
	IOReadBuffSize   uint32 `json:",default=1024"`  // The maximum size of the read buffer for each IO operation.(每次IO最大的读取长度)
	...
	RouterSlicesMode bool `json:",default=false"`

	// 是否开启 Request 对象池模式
	RequestPoolMode bool `json:",default=false"`
	/*
		logger
	*/
	LogDir string `json:",default=./log"` // The directory where log files are stored. The default value is "./log".(日志所在文件夹 默认"./log")

	// The name of the log file. If it is empty, the log information will be printed to stderr.
	// (日志文件名称   默认""  --如果没有设置日志文件，打印信息将打印至stderr)
	LogFile string `json:",default=zinx.log"`

	LogSaveDays int   `json:",default=7"`     // 日志最大保留天数
	LogFileSize int64 `json:",default=64"`    // 日志单个日志最大容量 默认 64MB,单位：字节，记得一定要换算成MB（1024 * 1024）
	LogCons     bool  `json:",default=false"` // 日志标准输出  默认 false
	...
	LogIsolationLevel int `json:",default=0"`
	...
	*/
	...
	HeartbeatMax int `json:",default=10"`

	/*
		TLS
	*/
	CertFile       string `json:",optional"` // The name of the certificate file. If it is empty, TLS encryption is not enabled.(证书文件名称 默认"")
	PrivateKeyFile string `json:",optional"` // The name of the private key file. If it is empty, TLS encryption is not enabled.(私钥文件名称 默认"" --如果没有设置证书和私钥文件，则不启用TLS加密)
}
...
func SetGlobalObject(obj *Config) {
	GlobalObject = obj
}
